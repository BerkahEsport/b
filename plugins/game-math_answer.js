...
handler.before = async function (m) {
    ...
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.text || !/^Pira hasile/i.test(m.quoted.text)) return !0
    this.math = this.math ? this.math : {}
    if (!(id in this.math)) return conn.sendButton(m.chat, '*Soale kui wes rampung!*', author, null, [['math', '/math']], m)
