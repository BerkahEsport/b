...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return conn.sendButton(m.chat, '*Soale kui wes rampung!*', author, ['siapakahaku', '/siapakahaku'], m)
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) m.reply(`*Sitik engkas!*`)
        else m.reply(`*Salah!*`)
    }
    return !0
}
